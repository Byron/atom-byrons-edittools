...
describe "text.plain", ->
  ...
  describe "at()", ->
    ...
    for [direction, offset, line] in [
      [previous, -1, "other   paragraph"]
      [next, +1, "yet another line"]
    ]
      ((direction, offset, line) ->
        it "moves from one empty line to the #{direction}", ->
          homeRow = 9
          b = block homeRow, 0
          expect(b.depth @editor).toBe 2
          nb = b.at direction, @editor

          expect(nb.range(@editor).start.row).toBe(homeRow + offset)
          expect(nb).toSelect line, @editor
      )(direction, offset, line)

    for [direction, homeRow] in [
      [previous, 0]
      [next, 12]
    ]
      ((direction, homeRow) ->
        it "should produce null if going to #{direction} from #{homeRow}", ->
          b = block homeRow, 0
          expect(b.depth @editor).toBe 2
          expect(b.at direction, @editor).toBe null
      )(direction, homeRow)
